...
class QiUnipa2_vision(Node):
    ...
    def get_emotion(self, request, response):
        ...
        try:
            ...
            if tracking_was_off:
                ...
                from qi_unipa_2_interfaces.srv import StartTracking
            ...
            if tracking_was_off:
                from qi_unipa_2_interfaces.srv import StopTracking
    ...
    def get_tracked_subject_coordinates(self, request, response):
        """
        Service per ottenere coordinate persona.
        Usa tracking come interruttore: avvia se spento, spegne alla fine.
        """
        tracking_was_off = False
        ...
        try:
            ...
            if tracking_was_off:
                self.get_logger().info("Avvio tracking People per ottenere coordinate...")
            ...
            self.get_logger().info(f"Stabilizzazione tracking '{self.active_target}'...")
            
            while len(stable_readings) < 3 and attempts < max_attempts:
                time.sleep(0.3)
                
                position = self.tracker.getTargetPosition(2)
                ...
                if position and len(position) >= 3:
                    ...
                    if 0.5 <= distance <= 5.0:
                        stable_readings.append((x, y, z))
                        self.get_logger().debug(
                            f"Lettura valida {len(stable_readings)}/3: ({x:.2f}, {y:.2f}, {z:.2f})m"
                        )
                
                attempts += 1
            
            # 3. VERIFICA STABILIZZAZIONE
            if len(stable_readings) < 3:
                response.success = False
                response.message = f"Tracking non stabilizzato dopo {attempts} tentativi"
                response.target_name = self.active_target or ""
                response.x = 0.0
                response.y = 0.0
                response.z = 0.0
                ...
                x_avg = sum(r[0] for r in stable_readings) / len(stable_readings)
                y_avg = sum(r[1] for r in stable_readings) / len(stable_readings)
                z_avg = sum(r[2] for r in stable_readings) / len(stable_readings)
                
                distance = math.sqrt(x_avg**2 + y_avg**2 + z_avg**2)
                theta = math.atan2(y_avg, x_avg)
                
                response.success = True
                response.message = f"Coordinate stabilizzate (media {len(stable_readings)} letture)"
                response.target_name = self.active_target or "People"
                response.x = float(x_avg)
                response.y = float(y_avg)
                response.z = float(z_avg)
                response.distance = float(distance)
                response.theta = float(theta)
                response.frame = "ROBOT"
                
                self.get_logger().info(
                    f"✓ Coordinate: ({x_avg:.2f}, {y_avg:.2f}, {z_avg:.2f})m, "
                    f"dist={distance:.2f}m, theta={theta:.2f}rad"
                )
            
        except Exception as e:
            response.success = False
            response.message = f"Errore tracking: {str(e)}"
            ...
            # 5. SPEGNI TRACKING (se era spento prima)
            if tracking_was_off:
                from qi_unipa_2_interfaces.srv import StopTracking
                req_stop = StopTracking.Request()
                res_stop = StopTracking.Response()
